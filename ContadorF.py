...
questions = ["RESUELVE: 1+1", "RESUELVE: 3-3", "RESUELVE: 1+0", "RESUELVE: 3-1", "RESUELVE: 7-3", "RESUELVE: 3+2", "RESUELVE: 4-3"]
answers = [2, 0, 1, 2, 4, 5, 1]
question_index = 0
     ...
     while True:
          ...
          if results.multi_hand_landmarks:
               ...
               for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    # Calcular el ángulo
                    value = (l1**2 + l3**2 - l2**2) / (2 * l1 * l3)
                    value = max(-1.0, min(1.0, value))
                    angle = degrees(acos(value))
          ...
          if question_index < len(questions):
               cv2.putText(frame, questions[question_index],(50, 110), 1, 2, (0, 0, 0), 2)
          
          # Comprobar respuesta
          if question_index < len(answers) and fingers_counter.isdigit() and int(fingers_counter) == answers[question_index]:
               if start_time is None:
                    start_time = time.time()
               if time.time() - start_time < 2:
                    cv2.putText(frame, "Respuesta correcta",(50, 150), 1, 2, (0, 255, 0), 2)
               else:
                    question_index += 1
                    start_time = None
          if question_index == len(questions):
               cv2.putText(frame, "FELICIDADES LO LOGRASTE",(50, 190), 1, 2, (0, 255, 0), 2)
               time.sleep(2)
               break
